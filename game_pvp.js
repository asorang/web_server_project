...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('opponent-rank-game').innerText = '(?급 | ????점)';
});
...
canvas.addEventListener('click', (event) => {
    ...
    // ★ (제거됨) 돌이 있는지 확인, 돌 그리기, 승리 판정, 턴 넘기기

    console.log(`플레이어 클릭: [${gridY}, ${gridX}]`);
    
    // (백엔드 연동) TODO: 웹소켓 서버가 완성되면 이 주석을 해제합니다.
    /*
    // (예시: 웹소켓으로 'move' 메시지 전송)
    // websocket.send(JSON.stringify({ type: 'move', move: [gridY, gridX] }));
    
    // (웹소켓으로 응답이 오면...)
    // websocket.onmessage = (event) => {
    //    const data = JSON.parse(event.data);
    //    if (data.type === 'opponent_move') {
    //        drawStone(data.move[0], data.move[1], 2); // 상대 돌 그리기
    //        currentPlayer = 'player';
    //    } else if (data.type === 'game_over') {
    //        showEndGameModal(data.message);
    //    }
    // }
    */
});
...
document.getElementById('resign-button').addEventListener('click', () => {
    ...
    if (isConfirmed) {
        alert('기권패했습니다. 상대방의 승리입니다!');
        // (백엔드 연동) TODO: 웹소켓으로 'resign' 메시지 전송
    }
});
...
rematchPvpButton.addEventListener('click', () => {
    ...
    // (백엔드 연동) TODO: 웹소켓으로 'rematch_request' 전송
    
    // (가짜 응답 - 2초 뒤 거절 - 이 부분도 제거하는 게 맞지만, 
    //  백엔드 개발자가 이 흐름을 이해하도록 임시로 남겨둘 수 있습니다.)
    setTimeout(() => {
